...
// es6，在非严格模式下可以声明块级函数，但是行为严格模式下的不同，严格模式下只能在当前块内有效，而非严格模式下函数不是提升到代码块的顶部而是提升至外围函数或全局作用域的顶部
// 剪头函数
// 没有this/super/arguments（可以通过命名参数或者不定参数访问函数的参数）/new target绑定{这些都是由外围最近一层非箭头函数决定，如果没有则this指向的是全局对象}；
// 不能通过new关键字调用；没有原型；
// 不可以改变this的绑定(即使是使用bind, apply, call)；
var test = '我是全局对象test';
var obj = {
  test: '我是局部变量test',
  myfun: function(){
...
// [[Enumerable]]: 表示数据是否可枚举：能否通过 for-in 循环返回属性。
// [[Writable]]: 表示能否修改属性的值。
...
console.log(children.description()); // 我是祖父hello
